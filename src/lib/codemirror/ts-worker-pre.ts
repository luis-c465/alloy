/**
 * Pre-request TypeScript Language Service Web Worker entry point.
 * Delegates to the shared worker implementation with scriptType = "pre".
 *
 * This is a separate file so that Vite can detect the static
 * `new URL('./ts-worker-pre.ts', import.meta.url)` pattern and
...
Comlink.expose(
  createWorker(() => {
    ...
    fsMap.set("/alloy.d.ts", getAlloyDeclaration("pre"));
